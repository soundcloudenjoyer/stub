...
typedef struct _Page {
	...
	int pageSize;
    //_Page() = default;
    //_Page(LPVOID addr, int size) : pageAdress(addr), pageSize(size) {}
} Page;

#define PAGE_SIZE 4096

inline std::list<Page> smallList;
//FILE *fp;
inline BYTE* finalPayload = NULL;
inline size_t finalPayloadSize;
inline IMAGE_SECTION_HEADER *SC_Header = NULL;
inline LPVOID ImageBase = NULL;
inline int section_count;
inline IMAGE_OPT_HEADER OPT_Header;
...
namespace stuff {
   ...
   // static BOOL deriveKey(const std::string &password, const BYTE* salt, BYTE* outKey);
    static BOOL deriveKeyBCrypt(const BCRYPT_ALG_HANDLE& algHandle, const std::string&password, const BYTE* salt, BYTE* outkey);

    BOOL decryptPayloadBCrypt(const std::string& password, const BYTE* encryptedPayload, size_t encryptedPayloadSize, BYTE* decryptedPayload, size_t* decryptedPayloadSize);
    ...
    LONG CALLBACK PageFaultHandler2(PEXCEPTION_POINTERS ExceptionsInfo);
};